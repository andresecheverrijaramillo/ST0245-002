...
public class Nodo{
    ...
    public Nodo(String [][] m){
        ...
        if(izq.length!=0&&der.length!=0){
            ...
            if(mostTrue){
                Izquierda = new Nodo(new Pair(10,"1")); 
                Derecha = new Nodo(new Pair(10,"1"));
            } else {
                Izquierda = new Nodo(new Pair(10,"0")); 
                Derecha = new Nodo(new Pair(10,"0"));
            }
        }
        /*
         * T(n) es c1+c2+2T(n+1)
    ...
    public boolean most(String [][] m,String value){
        int yes = 0;
        int no = 0;
        for(int i = 1; i<m.length;i++){ //c*n+c [N es el número de filas de la matriz]
            if(!m[i][10].equals(value)){ //c*n
